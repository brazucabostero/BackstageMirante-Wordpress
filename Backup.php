...
function getEventsHome()
{
  ...
  $public_events_ids = get_option('ingresse_public_events', true);

  $query = "
        SELECT 
            post_id, 
            meta_value,
            (select post_title from {$wpdb->posts} where ID = post_id) post_title,
            (select meta_value from {$wpdb->postmeta} _pm where _pm.post_id = pm.post_id and meta_key = 'mirante_event_session_datetime' limit 1 ) event_date
          FROM {$wpdb->postmeta} pm
        WHERE meta_key = 'mirante_event_id' and meta_value in ($public_events_ids) order by 4
  ";

  $results = $wpdb->get_results($query);

  $events = array_filter($results, function ($event) {
    $post = get_post($event->post_id);
    $sessions_datetimes = get_post_meta($event->post_id, 'mirante_event_session_datetimes', true);

    $event_is_valid = true;
    if (!is_array($sessions_datetimes)) {
      return false;
    }
    $first_event_date = $sessions_datetimes[array_key_last($sessions_datetimes)];
    $date = new DateTime($first_event_date);
    $date->setTime(0, 0, 0);
    ...
    if ($event_is_valid) return $event;
  });
...
?>
  <div class='list'>
    <?php foreach ($events as $event) : ?>
      ...
      $datetime_formated = $formatter->format($datetime);
      $city = get_post_meta($event->post_id, 'mirante_event_city', true);
      $street = get_post_meta($event->post_id, 'mirante_event_street', true);
      ?>
      <div class="ings-home-event-list-item">
        <div class="ings-home-event-list-item-show">
          <p class="data"><?= $datetime_formated; ?></p>
          <p class="show"><?= $event->post_title; ?></p>
        </div>
        ...
        <div class="ings-home-event-list-buttom button-wrapper">
          <a class="button" href="<?= get_permalink($event->post_id) ?>" target="_self">COMPRE AQUI <i class="fa fa-arrow-right"></i></a>
        </div>
      </div>
    <?php endforeach; ?>
  </div>
...
add_shortcode('testeEventos', 'getEventsHome');
